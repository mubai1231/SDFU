...
data_folder = os.path.join(os.path.dirname(os.path.abspath(__file__)), '../../data/oxford_flowers')
...
class FlowersFewShotInstance(ImageFolder):
...
class FlowersInstanceSample(Flowers):
...
def get_Flowers_train_transform(mean, std):
...
def get_Flowers_train_transform_strong(mean, std):
    normalize = transforms.Normalize(mean=mean, std=std)
...
def get_Flowers_test_transform(mean, std):
    normalize = transforms.Normalize(mean=mean, std=std)
    test_transform = transforms.Compose([transforms.Resize(256), transforms.CenterCrop(224), transforms.ToTensor(), normalize])
    return test_transform

def get_Flowers_dataloaders(batch_size, val_batch_size, num_workers, mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]):
    train_transform = get_Flowers_train_transform(mean, std)
    train_folder = os.path.join(data_folder, 'train')
    train_set = Flowers(train_folder, transform=train_transform)
    ...
    test_loader = get_Flowers_val_loader(val_batch_size, mean, std)
    return (train_loader, test_loader, num_data)

def get_fed_Flowers_dataloaders_strong(batch_size, val_batch_size, num_workers, cfg):
    mean = [0.485, 0.456, 0.406]
    std = [0.229, 0.224, 0.225]
    client_num = cfg.FEDERATED.CLIENT_NUM
    n_shot = cfg.FEDERATED.N_SHOT
    train_folder = os.path.join(data_folder, 'train')
    train_transform = get_Flowers_train_transform_strong(mean, std)
    train_dataset = FlowersFeb(root=train_folder, shots_per_client=cfg.FEDERATED.CLIENT_SHOTS, client_num=client_num, transform=train_transform, non_iid=cfg.FEDERATED.NON_IID, alpha=cfg.FEDERATED.ALPHA)
...
def get_Flowers_dataloaders_strong(batch_size, val_batch_size, num_workers, mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]):
    train_transform = get_Flowers_train_transform_strong(mean, std)
    train_folder = os.path.join(data_folder, 'train')
    train_set = FlowersFewShotInstance(train_folder, train_transform, n_shots=256)
    num_data = len(train_set)
    train_loader = torch.utils.data.DataLoader(train_set, batch_size=batch_size, shuffle=True, num_workers=num_workers, pin_memory=True)
    test_loader = get_Flowers_val_loader(val_batch_size, mean, std)
    return (train_loader, test_loader, num_data)

def get_Flowers_dataloaders_sample(batch_size, val_batch_size, num_workers, k=4096, mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]):
    train_transform = get_Flowers_train_transform(mean, std)
    train_folder = os.path.join(data_folder, 'train')
    train_set = FlowersInstanceSample(train_folder, transform=train_transform, is_sample=True, k=k)
    num_data = len(train_set)
    train_loader = torch.utils.data.DataLoader(train_set, batch_size=batch_size, shuffle=True, num_workers=num_workers, pin_memory=True)
    test_loader = get_Flowers_val_loader(val_batch_size, mean, std)
    return (train_loader, test_loader, num_data)

def get_Flowers_val_loader(val_batch_size, mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]):
    test_transform = get_Flowers_test_transform(mean, std)
