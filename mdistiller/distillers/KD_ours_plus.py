...
from ._common import kd_loss, cc_loss_pearson, bc_loss, mixup_data, mixup_data_conf


class KD_ours_plus(Distiller):
    def __init__(self, student, teacher, cfg):
        super(KD_ours_plus, self).__init__(student, teacher)
    ...
    def forward_train(self, image_weak, image_strong, target, **kwargs):
        ...
        loss_cc_weak = self.kd_loss_weight * ((cc_loss_pearson(
            ...
            logits_teacher_weak,
            self.temperature,
        ) * class_conf_mask).mean()) + self.kd_loss_weight * ((cc_loss_pearson(
            logits_student_weak,
            logits_teacher_weak,
            3.0,
        ) * class_conf_mask).mean()) + self.kd_loss_weight * ((cc_loss_pearson(
        ...
        loss_cc_strong = self.kd_loss_weight * cc_loss_pearson(
            logits_student_strong,
            logits_teacher_strong,
            self.temperature,
        ) + self.kd_loss_weight * cc_loss_pearson(
            logits_student_strong,
            logits_teacher_strong,
            3.0,
        ) + self.kd_loss_weight * cc_loss_pearson(
