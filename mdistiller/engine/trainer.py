...
class BaseTrainer(object):
    ...
    def set_distiller(self, distiller):
        self.optimizer = self.init_optimizer(self.cfg)
        self.best_acc = -1
        self.distiller = distiller
    ...
    def get_student(self):
        return load_checkpoint(os.path.join(self.log_path, "student_best"))["model"]
...
class AugTrainer(BaseTrainer):
    def train_iter(self, data, epoch, train_meters):
        ...
        acc1, acc5 = accuracy(preds, target, topk=(1, 1))
        train_meters["losses"].update(loss.cpu().detach().numpy().mean(), batch_size)
        train_meters["top1"].update(acc1[0], batch_size)
        train_meters["top5"].update(acc5[0], batch_size)
        msg = "Epoch:{}| Time(data):{:.3f}| Time(train):{:.3f}| Loss:{:.4f}| Top-1:{:.3f}| Top-5:{:.3f}".format(
...
class UnlearningTrainer(BaseTrainer):
    def train_iter(self, data, epoch, train_meters):
        self.optimizer.zero_grad()
        train_start_time = time.time()
        image, target, index = data
    ...
    def train_epoch(self, epoch):
        ...
        test_acc, test_acc_top5, test_loss = validate_clent_data(self.val_loader, self.distiller)
...
class FebAugTrainer(BaseTrainer):
    def train_iter(self, data, epoch, train_meters):
        self.optimizer.zero_grad()
        train_start_time = time.time()
        image, target, index = data
...
class FebTrainerBreakhis(FebAugTrainer):
    def train_epoch(self, epoch):
        lr = adjust_learning_rate(epoch, self.cfg, self.optimizer)
        train_meters = {
            "training_time": AverageMeter(),
        ...
        for val_loader in self.val_loader:
            test_acc_tmp, test_acc_top5_tmp, test_loss_tmp = validate(val_loader, self.distiller)
            test_acc += test_acc_tmp
            test_acc_top5 += test_acc_top5_tmp
            test_loss += test_acc_top5_tmp
        test_acc = test_acc / len(self.val_loader)
        test_acc_top5 = test_acc_top5 / len(self.val_loader)
        test_loss = test_loss / len(self.val_loader)
...
class FebUnlearningTrainerBreakhis(UnlearningTrainer):
    def train_epoch(self, epoch):
        lr = adjust_learning_rate(epoch, self.cfg, self.optimizer)
        train_meters = {
            "training_time": AverageMeter(),
        ...
        test_loss = 0
        for val_loader in self.val_loader:
            test_acc_tmp, test_acc_top5_tmp, test_loss_tmp = validate_clent_data(val_loader, self.distiller)
            test_acc += test_acc_tmp
            test_acc_top5 += test_acc_top5_tmp
            test_loss += test_acc_top5_tmp
        test_acc = test_acc / len(self.val_loader)
